...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (userData) {
      ...
      this.roleFromLocaleStorage = parsedUserData.role; // Affecter le rôle récupéré
    } else {
      console.log("Aucune donnée trouvée dans le localStorage pour 'user'.");
    }

    console.log(this.roleFromLocaleStorage); // Vérification dans la console
  ...
  handlefileinput(files: any) {
    //fonction bech ta9ra l image
    this.filetoupload = <ReadonlyArray<File>>files.target.files;
    console.log(this.filetoupload);
  }
  SeDeconnecter() {
    localStorage.clear();
    Swal.fire('Thank you...', 'You deconnected succesfully!', 'success');
    setTimeout(() => {
      location.reload();
    }, 1000);
  ...
  createuser() {
    ...
    this.authService.register(formdata).subscribe(
      (resutl: any) => {
        ...
        Swal.fire(
          'Succès ! Veuillez vérifier votre e-mail pour terminer votre inscription'
        );
        this.router.navigate(['/']);
      },
      (error) => {
